...
import com.example.st109_pdf_reader.databinding.ItemTypeFileSearchBinding

class TypeFileSearchAdapter(val context: Context) : RecyclerView.Adapter<TypeFileSearchAdapter.TypeFileVH>() {
    ...
    override fun onCreateViewHolder(
        ...
        return TypeFileVH(
            ItemTypeFileSearchBinding.inflate(
    ...
    inner class TypeFileVH(val binding: ItemTypeFileSearchBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(type: HomeAllFileModel, position: Int) {
            when (type.type) {
                KeyApp.ALL_FILE -> {
                    binding.tvType.text = context.getString(R.string.all)
                }

                KeyApp.WORD -> {
                    binding.tvType.text = context.getString(R.string.word)
                }

                KeyApp.EXCEL -> {
                    binding.tvType.text = context.getString(R.string.excel)
                }

                KeyApp.PPT -> {
                    binding.tvType.text = context.getString(R.string.ppt)
                }

                KeyApp.PDF -> {
                    binding.tvType.text = context.getString(R.string.pdf)
                }
            }
